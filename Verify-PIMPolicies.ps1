...
# Allow execution from any location by resolving full path
...
function Remove-JsonComments {
    ...
    # Remove inline // comments that are not inside quotes (simple state machine)
...
try {
    ...
    Write-Host "--- Raw Config (first 300 chars) ---" -ForegroundColor DarkGray
    Write-Host ($configRaw.Substring(0,[Math]::Min(300,$configRaw.Length)))
    Write-Host "--- Cleaned Config (first 300 chars) ---" -ForegroundColor DarkGray
    Write-Host ($clean.Substring(0,[Math]::Min(300,$clean.Length)))
    throw "Failed to parse config: $($_.Exception.Message)"
}

if (-not $json) { throw "Parsed JSON object is null - check for trailing commas or invalid comment removal." }

function Get-ResolvedPolicyObject {
    param($p) ; if ($p.PSObject.Properties['ResolvedPolicy'] -and $p.ResolvedPolicy) { return $p.ResolvedPolicy } ; return $p }

# --- Normalize / extract expected policy objects from various supported schema shapes ---
$expectedAzure = @()
$expectedEntra = @()
$expectedGroup = @()
...
if ($json.PSObject.Properties['PolicyTemplates']) {
    # Convert PSCustomObject templates to hashtable for ContainsKey
    $templates = @{}
    foreach ($t in ($json.PolicyTemplates | Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name)) {
        $templates[$t] = $json.PolicyTemplates.$t
    }
}
...
function Resolve-Template {
    ...
    if ($obj.Template -and $templates.ContainsKey($obj.Template)) {
        $base = $templates[$obj.Template] | ConvertTo-Json -Depth 20 | ConvertFrom-Json
        # Merge: template first then specific overrides
        foreach ($p in $obj.PSObject.Properties) { $base | Add-Member -NotePropertyName $p.Name -NotePropertyValue $p.Value -Force }
        return $base
    }
    return $obj
}

# 1. Direct array properties (legacy): AzureRolePolicies / EntraRolePolicies / GroupPolicies
...
# 2. New nested maps: AzureRoles.Policies (roleName->policy), EntraRoles.Policies, GroupRoles.Policies (groupId -> roleName -> policy)
if ($json.PSObject.Properties['AzureRoles'] -and $json.AzureRoles.PSObject.Properties['Policies']) {
    foreach ($prop in $json.AzureRoles.Policies.PSObject.Properties) {
        $roleName = $prop.Name; $p = $prop.Value
        if (-not $p) { continue }
...
if ($json.PSObject.Properties['GroupRoles'] -and $json.GroupRoles.PSObject.Properties['Policies']) {
    foreach ($gprop in $json.GroupRoles.Policies.PSObject.Properties) {
        $groupId = $gprop.Name; $roleBlock = $gprop.Value
        if (-not $roleBlock) { continue }
        foreach ($rprop in $roleBlock.PSObject.Properties) {
            $roleName = $rprop.Name; $p = $rprop.Value
            if (-not $p) { continue }
...
# Resolve templates into concrete properties
...
# Live property name mapping for Entra role policies (module returns different names)
$liveNameMap = @{
...
Write-Host ("Loaded expected counts -> Azure:{0} Entra:{1} Group:{2}" -f $expectedAzure.Count,$expectedEntra.Count,$expectedGroup.Count) -ForegroundColor DarkCyan

function Compare-Policy {
    param(
        [string]$Type,[string]$Name,[object]$Expected,[object]$Live,[string]$ExtraId=$null,[int]$ApproverCountExpected=$null
    )
    $differences = @()
    foreach ($f in $fields) {
        if ($Expected.PSObject.Properties[$f]) {
            $exp = $Expected.$f
            $liveProp = $f
            if ($liveNameMap.ContainsKey($f)) { $liveProp = $liveNameMap[$f] }
            $liveVal = $null
            if ($Live -and $Live.PSObject -and $Live.PSObject.Properties[$liveProp]) { $liveVal = $Live.$liveProp }
    ...
    if ($null -ne $ApproverCountExpected -and $Expected.PSObject.Properties['ApprovalRequired'] -and $Expected.ApprovalRequired) {
        ...
        foreach ($aprop in 'Approvers','Approver','Approval','approval','ApproverCount') {
            if ($Live.PSObject.Properties[$aprop]) {
                $val = $Live.$aprop
                ...
                if ($liveApproverCount -ne $null) { break }
            }
        }
...
if (-not (Get-Command Get-PIMEntraRolePolicy -ErrorAction SilentlyContinue)) {
...
if ($expectedAzure.Count -gt 0 -and -not $SubscriptionId) {
    ...
    foreach ($p in $expectedAzure) {
        ...
        if (-not $p.Scope) { $results += [pscustomobject]@{ Type='AzureRole'; Name=$p.RoleName; Target='(missing scope)'; Status='Error'; Differences='Missing Scope' }; $script:driftCount++; continue }
        try {
            ...
            $script:results += [pscustomobject]@{ Type='AzureRole'; Name=$p.RoleName; Target=$p.Scope; Status='Error'; Differences=$_.Exception.Message }
            $script:driftCount++
        }
    }
}
...
foreach ($p in $expectedEntra) {
    ...
    $script:results += [pscustomobject]@{ Type='EntraRole'; Name=$p.RoleName; Target='/'; Status='Error'; Differences=$_.Exception.Message }
        $script:driftCount++
    }
}
...
foreach ($p in $expectedGroup) {
    $r = Get-ResolvedPolicyObject $p
    # Normalize expected properties for comparison (mirror application normalization)
    if (-not $r.PSObject.Properties['ActivationRequirement'] -and $r.PSObject.Properties['EnablementRules'] -and $r.EnablementRules) {
        try { $r | Add-Member -NotePropertyName ActivationRequirement -NotePropertyValue $r.EnablementRules -Force } catch { $r.ActivationRequirement = $r.EnablementRules }
    }
    if (-not $r.PSObject.Properties['ActivationDuration'] -and $r.PSObject.Properties['Duration'] -and $r.Duration) {
        try { $r | Add-Member -NotePropertyName ActivationDuration -NotePropertyValue $r.Duration -Force } catch { $r.ActivationDuration = $r.Duration }
    }
    # Support either GroupId or GroupName; if only Id present that's enough
    ...
    $script:results += [pscustomobject]@{ Type='Group'; Name=$p.RoleName; Target=$gid; Status='Error'; Differences=$_.Exception.Message }
        $script:driftCount++
    }
}

Write-Host "`nPolicy Verification Results:" -ForegroundColor Cyan
...
if ($script:results.Count -eq 0) {
    Write-Host "⚠️ No policies discovered in config (nothing compared)." -ForegroundColor Yellow
} else {
    ...
    else { Write-Host "❌ Drift detected in $script:driftCount policy item(s)." -ForegroundColor Yellow; if ($FailOnDrift) { exit 1 } }
}
