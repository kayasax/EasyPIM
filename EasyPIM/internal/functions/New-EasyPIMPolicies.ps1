                        ...
                        try {
                            ...
                            if ($policyResult.Status -like "*Protected*") {
                                ...
                                if ($resolved) {
                                    ...
                                    if ($notifCount -eq 0 -and $resolved.PSObject.Properties['Notifications'] -and $resolved.Notifications) {
                                        $nc = 0
                                        foreach ($area in 'Eligibility','Active','Activation') {
                                            if ($resolved.Notifications.PSObject.Properties[$area] -and $resolved.Notifications.$area) {
                                                foreach ($sub in 'Alert','Assignee','Approvers') {
                                                    if ($resolved.Notifications.$area.PSObject.Properties[$sub] -and $resolved.Notifications.$area.$sub) { $nc++ }
                                                }
                                            }
                                        }
                                        $notifCount = $nc
...
function Set-GroupPolicy {
    ...
    # Flatten nested Notifications from templates into Notification_* keys expected by Set-PIMGroupPolicy
    try {
        ...
        switch ($prop.Name) {
            ...
            'ActiveAssignmentRequirement' {
                if ($null -ne $prop.Value) {
                    $val = $prop.Value
                    if ($val -is [string]) {
                        if ($val -match ',') { $val = ($val -split ',') | ForEach-Object { $_.ToString().Trim() } }
                        else { $val = @($val) }
                    }
                    elseif (-not ($val -is [System.Collections.IEnumerable])) { $val = @($val) }
