<#
    .Synopsis
        Import the settings from the csv file $path for PIM Group policies
    .Description
        Convert the csv back to group policy rules
    .Parameter Path
        path to the csv file
    .Example
        PS> Import-GroupSettings -path "c:\temp\mygroup.csv"

        Import settings from file c:\temp\mygroup.csv

    .Notes
        Author: Loïc MICHEL
        Homepage: https://github.com/kayasax/EasyPIM
     #>
function Import-GroupSettings {
    [CmdletBinding()]
    ...
    log "Importing group settings from $path" -noEcho
    if (!(test-path $path)) {
    ...
    $csv | ForEach-Object {
        ...
        # Filter enablement rules for Admin Assignment
        # Allowed: Justification, MultiFactorAuthentication (Ticketing is ONLY for end-user activation)
        $activeAssignmentRequirements = Split-OrEmpty $_.ActiveAssignmentRules
        ...
        # Resolve policy ID from GroupID and RoleName (type: member/owner) if PolicyID missing
        $policyIdToUse = $_.PolicyID
        if ([string]::IsNullOrWhiteSpace([string]$policyIdToUse)) {
            $gid  = $_.GroupID
            $type = $_.RoleName
            if (-not [string]::IsNullOrWhiteSpace($gid) -and -not [string]::IsNullOrWhiteSpace($type)) {
                $assignEp = "policies/roleManagementPolicyAssignments?`$filter=scopeId eq '$gid' and scopeType eq 'Group' and roleDefinitionId eq '$($type.ToLower())'&`$expand=policy(`$expand=rules)"
                $assign = invoke-graph -Endpoint $assignEp -ErrorAction Stop
                $policyIdToUse = $assign.value[0].policyId
                if (-not $policyIdToUse) { $policyIdToUse = $assign.value[0].policyID }
            }
        }
        if ([string]::IsNullOrWhiteSpace([string]$policyIdToUse)) { throw "Unable to resolve PolicyID for GroupID='$($_.GroupID)' RoleName='$($_.RoleName)'" }
