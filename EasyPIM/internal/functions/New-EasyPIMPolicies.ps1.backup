                    ...
                    foreach ($policyDef in $Config.AzureRolePolicies) {
                        ...
                        try {
                            ...
                            if ($policyResult.Status -like "*Protected*") {
                                ...
                                Write-Host "  🛡️ Protected Azure role '$($policyDef.RoleName)' - policy change blocked for security" -ForegroundColor Yellow
                            } else {
                                $results.Summary.Successful++
                                if ($PolicyMode -eq "validate") {
                                    Write-Host "  ✅ Validated policy for role '$($policyDef.RoleName)' at scope '$($policyDef.Scope)' (no changes applied)" -ForegroundColor Green
                                } else {
            ...
            if ($PSCmdlet.ShouldProcess($whatIfMessage, "Entra Role Policies")) {
                ...
                foreach ($policyDef in $Config.EntraRolePolicies) {
                    ...
                    try {
                        ...
                        if ($policyResult.Status -like "*Protected*") {
                            $results.Summary.Skipped++
                            Write-Host "  🛡️ Protected role '$($policyDef.RoleName)' - policy change blocked for security" -ForegroundColor Yellow
                        } else {
                            $results.Summary.Successful++
                            if ($PolicyMode -eq "validate") {
                                Write-Host "  ✅ Validated policy for Entra role '$($policyDef.RoleName)' (no changes applied)" -ForegroundColor Green
                            } else {
            ...
            if ($PSCmdlet.ShouldProcess($whatIfMessage, "Group Policies")) {
                ...
                foreach ($policyDef in $Config.GroupPolicies) {
                    ...
                    try {
                        ...
                        if ($policyResult.Status -like "*Protected*") {
                            $results.Summary.Skipped++
                            Write-Host "  🛡️ Protected Group '$($policyDef.GroupId)' role '$($policyDef.RoleName)' - policy change blocked for security" -ForegroundColor Yellow
                        } else {
                            $results.Summary.Successful++
                            if ($PolicyMode -eq "validate") {
                                Write-Host "  ✅ Validated policy for Group '$($policyDef.GroupId)' role '$($policyDef.RoleName)' (no changes applied)" -ForegroundColor Green
                            } else {
...
function Set-AzureRolePolicy {
    ...
    if ($protectedAzureRoles -contains $PolicyDefinition.RoleName) {
        $warningMsg = "⚠️ PROTECTED AZURE ROLE: '$($PolicyDefinition.RoleName)' is a critical role. Policy changes are blocked for security."
        Write-Warning $warningMsg
        Write-Host "🛡️ Protected Azure role '$($PolicyDefinition.RoleName)' - policy change blocked" -ForegroundColor Yellow
    ...
    if ($Mode -eq "validate") {
        ...
        Write-Host "📋 Policy Changes for Azure Role '$($PolicyDefinition.RoleName)' at '$($PolicyDefinition.Scope)':" -ForegroundColor Cyan
        Write-Host "   ⏰ Activation Duration: $($policy.ActivationDuration)" -ForegroundColor Yellow
        Write-Host "   🔐 Activation Requirements: $($policy.ActivationRequirement)" -ForegroundColor Yellow
        if ($policy.ActiveAssignmentRequirement) {
            Write-Host "   🔒 Active Assignment Requirements: $($policy.ActiveAssignmentRequirement)" -ForegroundColor Yellow
        }
        Write-Host "   ✅ Approval Required: $($policy.ApprovalRequired)" -ForegroundColor Yellow
        if ($policy.Approvers -and $policy.ApprovalRequired -eq $true) {
            Write-Host "   👥 Approvers: $($policy.Approvers.Count) configured" -ForegroundColor Yellow
        }
        Write-Host "   🎯 Max Eligibility Duration: $($policy.MaximumEligibilityDuration)" -ForegroundColor Yellow
        Write-Host "   ⚡ Max Active Duration: $($policy.MaximumActiveAssignmentDuration)" -ForegroundColor Yellow
        ...
        if ($notificationCount -gt 0) {
            Write-Host "   📧 Notification Settings: $notificationCount configured" -ForegroundColor Yellow
        }

        Write-Host "   ⚠️  NOTE: No changes applied in validation mode" -ForegroundColor Magenta
...
function Set-EntraRolePolicy {
    ...
    if ($Mode -eq "validate") {
        ...
        # Show policy details that would be applied
        $policy = $PolicyDefinition.ResolvedPolicy
        Write-Host "📋 Policy Changes for Entra Role '$($PolicyDefinition.RoleName)':" -ForegroundColor Cyan
        Write-Host "   ⏰ Activation Duration: $($policy.ActivationDuration)" -ForegroundColor Yellow
        Write-Host "   🔐 Activation Requirements: $($policy.ActivationRequirement)" -ForegroundColor Yellow
        if ($policy.ActiveAssignmentRequirement) {
            Write-Host "   🔒 Active Assignment Requirements: $($policy.ActiveAssignmentRequirement)" -ForegroundColor Yellow
        ...
        if ($policy.AuthenticationContext_Enabled -eq $true) {
            Write-Host "   🛡️ Auth Context: $($policy.AuthenticationContext_Value)" -ForegroundColor Yellow
        }
